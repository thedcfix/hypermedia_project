...
switch ($_POST['method']) {
    ...
    case 'in_evidenza_min_max':
        $conn = mysqli_connect("localhost", "root", "");
        mysqli_select_db($conn, "my_reyze");
        $res = productsShowMinMaxEvid($conn,$_POST['min'],$_POST['max']);
        while($row = mysqli_fetch_array($res)) {
            $var = $row['id'];
            $imma = mysqli_query($conn, "select img from immagini where disp_id = $var");
            $img =  mysqli_fetch_array($imma);
...
function productsShowMinMaxEvid($conn,$min,$max) {

    $res = mysqli_query($conn, "select * from prodotti where in_evidenza = true && prezzo < $max && prezzo > $min order by Prezzo DESC");
